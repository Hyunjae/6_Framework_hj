...
//	@Scheduled(fixedRate=10000) // 이전 작업 시작 후 10초 뒤에 수행
...
//	@Scheduled(cron = "0/15 * * * * *") //  0초부터 시작하여 15초마다
	...
	@Autowired
	private ServletContext application; // application scpoe 객체
	...
	public void deleteImageFile() { // 매개변수 작성불가, 작성시 오류남
...
//				= file.toString().substring(file.toString().lastIndexOf("/")+1);
				// 파일명.jpg (/ 뒤를 조회하고 +1로 /빼고 그 뒤에만 조회)
				= file.getName();
