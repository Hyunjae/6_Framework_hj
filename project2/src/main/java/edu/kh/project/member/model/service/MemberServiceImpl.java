...
//	MemberDAO dao = new MemberDAO();
	 ...
	 * 	- 클래스, 메서드 위에 작성 가능 (메서드 위에 작성하는 것을 권장)
	 * 	  (클래스 위에 작성하면 필요없는 select같은 구문도 다 commit시킴)
	 * 	- 조건 : AOP를 이용한 기술 -> Service Interface 필요
	 * 			트랜잭션 매니저가 bean으로 등록되어야 한다(root-context.xml)
	 * 
	 * @Transactional 어노테이션 특징
	 * - 예외가 발생한 경우 rollback 자동 진행
	 * - 예외의 기본값은 RuntimeException
	 * 		-> SQL 진행 시 발생하는 예외 == SQLException(RuntimeException의 형제, 다형성 적용X)
	 * 		-> RuntimeException 외에 다른 예외에도 rollback이 진행되고 싶다면
	 * 			rollbackFor = 예외클래스를 작성하면 된다
	 * */
