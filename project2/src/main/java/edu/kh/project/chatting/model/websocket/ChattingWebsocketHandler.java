...
public class ChattingWebsocketHandler extends TextWebSocketHandler {
	...
	// --> servlet-context.xml에서 연결 -> 핸들러 클래스 매핑 -> 해당 메서드 실행
	@Override
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if(result>0) { // 삽입 성공 시
			...
			// msg 객체(채팅방 번호, 대상번호, 내용, 보낸사람 번호, 보낸사람 번호, 보낸 시간)
			// -> JSON으로 변환 해서 로그인한 회원 중 
			// 대상번호, 보낸사람 번호가 일치하는 2명에게 웹소켓으로 메세지전달
...
/* WebSocket
- 브라우저와 웹서버간의 전이중통신(양방향 통신)을 지원하는 프로토콜이다
- HTML5버전부터 지원하는 기능이다.
