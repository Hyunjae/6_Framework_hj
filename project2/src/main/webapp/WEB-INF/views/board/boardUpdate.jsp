...
<body>
    <main>
        ...
        <form action="update" method="POST" enctype="multipart/form-data" class="board-write" id="boardWriteForm" onsubmit="return updateValidate()">
            ...
            <h5>업로드 이미지</h5>
            <div class="img-box">
                <%-- 이미지가 미리보기만 나오고 있고 실제 input에는 비어있음
                    1. 기존 이미지 O -> 삭제한 경우(x버튼 누름)
                    : 어떤 이미지가 삭제되었는지 확인, DB에서 삭제
                    2. 기존 O -> 새로운 이미지 : 삭제 후 수정
                    3. 기존 X -> 새로운 이미지 : 삽입
                    * <input type="file>은 기존 이미지 경로를 넣을 수 없다
                --%>
                <div class="boardImg">
                    <label for="img1">
